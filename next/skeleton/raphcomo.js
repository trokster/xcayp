...
//Hack to allow eve.off on eve.once
//It's ugly, maybe ask for help on this one
...
//https://raw.githubusercontent.com/math-utils/linear-partition/master/linear_partition.js
(function(glob){
    ...
    window.linear_partition= linear_partition;
})()
...
eve.on("database.login.request", function(database, custom){
    ...
    login.init = function(){
        ...
        var handleInputChange = function(){
            ...
            if(isUndefinedOrNull(usr) || usr == ""){
                ...
                if(!custom){
                    ...
                    try {
                      database.changes_canceller.cancel();
                      console.log("Canceller called on DB: " + database.local);
                    } catch(e){
                      console.log("Error calling canceller: " + e);
                      console.log(e);
                      noop();
                    }
        ...
        connect(self.username_input, "onkeypress", function(e){
          window.evt = e;
          if(e.key().string == "KEY_ENTER" || e.key().string == "\r") {
            e.stop();
            handleInputChange();
          }
        });
        connect(self.password_input, "onkeypress", function(e){
          if(e.key().string == "KEY_ENTER" || e.key().string == "\r") {
            e.stop();
            handleInputChange(); 
          }
...
(function(glob) {
    eve.on("database.request.*", function() {
                ...
                if(databases[db].sync.to && !isUndefinedOrNull(databases[db].remote)) {
                    var onComplete = function(err, res) {
                        ...
                        if(!isUndefinedOrNull(err)) {
                            ...
                            if(err.other_errors && isArrayLike(err.other_errors) && err.other_errors[0].message == "Only database admin can write here, seriously :)."){
        ...
        if(type == "changes") {
            ...
            var continuous = databases[db].database.type() != 'http' ? true : false;
            databases[db].changes_canceller = databases[db].database.changes({
                "onChange": function(change) {
                    //Handle bug on last seq
                    //if(change.seq <= databases[db].last_seq) return;
                    //log("DB: " + db + " CHANGE: " + serializeJSON(change), -1);
                    eve("database.change." + db + "." + change.id, change);
                ...
                "complete": function(err, res) {
                    ...
                    if(!continuous) eve("buffered." + databases[db].polling + ".database.request.changes." + db);
                },
...
eve.on("interface.request_handle.*", function() {
    ...
    //TODO: make it so this dies after a thousand or so iterations and no response
    var evt = eve.nt();
    ...
    if (!isUndefinedOrNull(err)) {
      ...
      db.get("user_init", function(err, doc) {
        if (!isUndefinedOrNull(err)) {
          ...
          eve("database.request.changes.userdb");
          //Attempt to update local credentials
          eve("buffered.500.application.refresh_credentials");
        }
      });
    }
  });
        ...
        if(doc){
          ...
          database.database = new PouchDB(PouchDB.utils.Crypto.MD5(database.local), function(err, db) {
            if (!isUndefinedOrNull(err)) {
              ...
              eve("delayed.100.database.request.changes.userdb");
              eve("buffered.500.application.refresh_credentials");
            }
          });
        }
...
//Setup up user_credentials for application
eve.on("application.refresh_credentials", function(){
  //Fetch local credentials
  databases.userdb.database.get("user_init", function(err, user_init){
    if(err){
      console.log("Could not fetch user_init from userdb.");
    }
    else {
      window.USER_CREDENTIALS = user_init;
      //Attempt to log on to remote DB userdb
      var db = rhost.split("://");
      PouchDB(db[0] + "://" + databases.userdb.auth.username + ":" + databases.userdb.auth.password + "@" + db[1] + "_users", function(err, usersdb) {
        if(err){
          console.log("Error connecting to usersdb(remote)");
        }
        else {
          usersdb.get("org.couchdb.user:" + databases.userdb.auth.username, function(err, usr){
            user_init.credentials = usr;
            databases.userdb.database.put(user_init);
            window.USER_CREDENTIALS = user_init;
          });
        }
      })
...
//Handle potential rollback for an interface object
eve.on("interface.rollback_request", function(database, doc_name){
  database.get(doc_name).then(function(doc){
    if(doc.previous_content) {
      doc.content = doc.previous_content;
      doc.previous_content = null;
      delete doc.previous_content;
      database.put(doc).then(function(){
        console.log("rollback success")
      });
    }
    else {
      console.log("Sorry, no previous content saved for that document");
    }
  });
});
