  ...
  for(var property in obj) {
    ...
    if(typeof value == 'string' && value.slice(0,8) == "function") {
      ...
      if(value instanceof Array) {
        ...
        var res = map(function(val){ if(typeof val == 'string') return "'"+val+"'" ;return val; }, value);
...
eve.on("database.init", function(){
    ...
    try {
        ...
        PouchDB("idb://" + db.local, {auto_compaction:true}, function(err, database) {
        ...
        try {
            PouchDB("websql://" + db.local, {auto_compaction:true}, function(err, database) {
                eve("database.created."+name, {
                    "error": err,
                    "database": database,
                    name: name
        ...
        db.local_db_string = "websql://" + db.local;
            ...
            try {
                ...
                db.nosync = true;
                database_tgt = PouchDB(db.remote,function(err, database) {
                    eve("database.created."+name, {
                        "error": err,
                        "database": database,
                        name: name
