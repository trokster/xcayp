...
(function() {
  ...
  var stem = function(revtree, depth){
    ...
    wTree.map(function(tree){
      ...
      var max = 0;
      all.map(function(node){
          if( node.depth > max ) max = node.depth;
      });
      if(max>depth){
          all.filter(function(node){return node.depth == ( max-depth+1 );})
              .map(function(node){
                  resTree.push({ids:node.node, pos:tree.pos+max-depth});
          });
      }
      else {
          resTree.push(tree);
  ...
  Pouch.merge = function(metadata, path, depth){
    ...
    var looped = 0;
    var to_place = [];
    bfs({node:path.ids, depth:1}, allNodes)
      .map(function(node){
    ...
    while(to_place.length > 0){
      ...
      //We're running a BFS,so for each node, we search a match,
      //and add sons when necessary
      var n1 = null;
      for (var i=0;i<metaNodes.length;i++){
          var n = metaNodes[i];
          //We take it for granted rev is unique per document
          if(n.node[0] == node.node[0]) {
            n1 = metaNodes[i];
            break;
          }
      }
      if(n1){
        //Node exists, we consider it a merge, keep going
        //log("NODE exists, skipping");
        continue;
      }
      else {
        //Not found
        ...
        if(node.parent){
          ...
          for (var i=0;i<metaNodes.length;i++){
              var n = metaNodes[i];
              //We take it granted rev is unique per document
              if(n.node[0] == node.parent.node[0]) {
                n1 = metaNodes[i];
                break;
              }
          }
          if(!n1){
            //log("ERRRM: parent not found: " + node.parent.node[0]);
            to_place.unshift(node);
            continue;
          }
          //We add this node to parent's children
          n1.node[1].push([node.node[0], []]);
          ...
          metadata.rev_tree.push({
            "pos": path.pos,
            "ids": [node.node[0],[]]
          });

          metaNodes.push({depth:1, node: metadata.rev_tree[metadata.rev_tree.length-1].ids, parent:null, isLeaf: node.node[1].length == 0 });
    ...
    return {metadata:metadata, conflicts: conflicts};
  }
    ...
    fetchCheckpoint(src, target, function(checkpoint) {
      ...
      var repOpts = {
        ...
        onChange: function(change) {
          ...
          if(change.id && change.id.indexOf("_local") == 0) {
            if(window.LOG_INFO_REPLICATION) log2("Ignoring: " + change.id);
            pending--;
            isCompleted();
            return;
          }
          if(change.id && change.id.indexOf("_design") == 0) {
            if(window.LOG_INFO_REPLICATION) log2("Ignoring: " + change.id);
            pending--;
            isCompleted();
            return;
          ...
          target.revsDiff(diff, function(err, diffs) {
            ...
            //log2("Diffs found: " + JSON.stringify(diffs));
            Object.keys(diffs).map(function(id) {
              //log2("Processing: " + id);
              diffs[id].missing.map(function(rev) {
                //log2("Getting: " + id + " :: " + rev);
                var being_fetched = id + " :: " + rev;
                if(window.LOG_INFO_REPLICATION) log2("Processing: " + being_fetched);
                revs_remaining.push(being_fetched);
                total_to_complete++;
                src.get(id, {
                  revs: true,
                  rev: rev,
                  ...
                  if(err) {
                    if(window.LOG_INFO_REPLICATION) log2("Encountered error: " + being_fetched + JSON.stringify(err));
                  }
                  var being_processed = doc;
                  target.bulkDocs({
                    docs: [doc]
                  }, {
                    new_edits: false
                  }, function(err, docs, xhr) {
                    total_completed++;
                    ...
                    if(err) {
                      ...
                      if(err && err.reason && err.reason.error && err.reason.error.name == "ConstraintError") {
                        noop();
                      } else if((target.type && (target.type() == "idb" || target.type() == "websql")) && err.status == 500) {
                        if(window.LOG_FAILED_REPLICATION) log2("LOG: Failed to save locally: " + JSON.stringify([err, doc]));
                      } else {
                        result.ok = false;
                        result.errors.push({
                          error: err,
...
// -Hans
//Implementation of a non recursive tree manipulation library
var allNodes = function(node, basket){
    if(typeof(node.node[0]) == "string"){
        basket.push({depth:node.depth, node:node.node, parent:node.parent ? node.parent : null, isLeaf: node.node[1].length == 0 });
        if(node.node[1].length > 0){
            return node.node[1].map(function(n){
                return {node:n, depth:node.depth+1, parent:node}
            });
        }
    }
};

//Node at first will be of the form
//{node:node, depth:depth}
//destructive function
//Careful with what you do with the result
var bfs = function(node, visitor){
  to_visit = [node];
  var basket = [];
  while(to_visit.length>0){
    var res = visitor(to_visit.pop(), basket);
    if(res){
    res.map(function(o){
      to_visit.unshift(o);
      });
...
//Very careful with this one, it returns a POINTER
//to the actual tree
var findRev = function(metadata, pos, rev){
    var revs = metadata.rev_tree;
    var nodes = [];
    if(!metadata.deletions) metadata.deletions = {};
    revs.map(function(tree){
        bfs({node:tree.ids, depth:1}, allNodes)
            .filter(function(node){
                return (node.node[0] == rev) && (tree.pos+node.depth-1) == pos;
            })
            .map(function(node){
                nodes.push({"rev":(tree.pos+node.depth-1) + "-" + node.node[0], "deleted": node.node[0] in metadata.deletions, pnode: node, treepos: tree.pos });
        });
    });
    if(nodes.length > 1) {
      log("ERROR: more than 1 matches to a rev search: " + JSON.stringify({metadata:metadata, rev: rev}), 1);
    }
    return nodes[0];
}
...
var collectLeaves = function(metadata){
    var revs = metadata.rev_tree;
    var leaves = [];
    if(!metadata.deletions) metadata.deletions = {};
    revs.map(function(tree){
        bfs({node:tree.ids, depth:1}, allNodes)
            ...
            .map(function(node){
                leaves.push({pos:(tree.pos+node.depth-1), id:node.node[0], "deleted": node.node[0] in metadata.deletions });
        });
    });
    leaves.sort(compareRevs);
    leaves = leaves.map(function(item){return {rev:item.pos+"-"+item.id};});
    //log("LEAVES: " + JSON.stringify(leaves));
    return leaves;
}
var collectConflicts = function(metadata) {
  var leaves = collectLeaves(metadata);
...
var winningRev = function(metadata) {
  return collectLeaves(metadata)[0].rev;
}
...
var ajax = function ajax(options, callback, count) {
  if(!count) count=0;
  if(options)
    window.AJAX_QUEUE.unshift([options, callback, count]);

  if(window.AJAX_CURRENT_OPEN_REQUESTS >= window.AJAX_CONCURRENT_REQUEST_LIMIT){
    //log2("Request queued");
    return;
  }
  if(window.AJAX_QUEUE.length==0) return;
  window.AJAX_CURRENT_OPEN_REQUESTS++;
  
  var opts = window.AJAX_QUEUE.pop();
  options = opts[0];
  callback = opts[1];
  count = opts[2];
  opts = null;
  ...
  if(count>1){
    if(window.LOG_INFO_REPLICATION)
      log2("AJAX RESUB: " + count + " :: " + JSON.stringify(options));
  }
  ...
  var success = function sucess(obj, _, xhr) {
    ...
    if(options.type != "HEAD" && options.dataType && options.dataType == "json" && obj === null){
      //Something wrong happened during ajax operation
      //re-submit
      window.AJAX_QUEUE.unshift([options, callback, count]);
      return;
    }
...
var HttpPouch = function(opts, callback) {
    ...
    api.get = function(id, opts, callback) {
      //log2("API GET : " + id + " :: " + JSON.stringify(opts));
      ...
      ajax(options, function(err, doc, xhr) {
        ...
        if(err) {
          //log2("API GET ERROR: ERR :: " + JSON.stringify(err));
          //log2("API GET ERROR: DOC :: " + JSON.stringify(doc));
          //log2("API GET ERROR: XHR :: " + JSON.stringify(xhr));
...
var IdbPouch = function(opts, callback) {
      ...
      function complete(event) {
        ...
        results.forEach(function(result) {
          ...
          var change = {
            id: metadata.id,
            seq: metadata.seq,
            changes: collectLeaves(metadata),
            doc: result.data
          };
          change.doc._rev = rev;
          update_seq++;
      ...
      txn.objectStore(DOC_STORE).get(id.docId).onsuccess = function(e) {
        ...
        index.get(key).onsuccess = function(e) {
          ...
          if(opts.revs) {
            ...
            while(node.pnode.parent) {
              revisions2.ids.push(node.pnode.node[0]);
              node = {
                pnode: node.pnode.parent
              };
            }
            revisions2.ids.push(node.pnode.node[0]);
            //revisions2.ids.reverse();
            revisions2.start += revisions2.ids.length - 1;
            doc._revisions = revisions2;
            delete node;
          }
          if(opts.revs_info) {
            var node = findRev(metadata, doc._rev.split('-')[0], doc._rev.split('-')[1]);
            ...
            while(node.pnode.parent) {
              revisions2.push({
                rev: (node.treepos + node.pnode.depth - 1) + "-" + node.pnode.node[0],
                status: "available"
              });
              node = {
                pnode: node.pnode.parent,
                treepos: node.treepos
              };
            }
            revisions2.push({
              rev: (node.treepos + node.pnode.depth - 1) + "-" + node.pnode.node[0],
    ...
    api.changes = function idb_changes(opts, callback) {
      ...
      function fetchChanges() {
        ...
        var req = descending ? txn.objectStore(BY_SEQ_STORE).openCursor(IDBKeyRange.lowerBound(opts.seq, true), descending) : txn.objectStore(BY_SEQ_STORE).openCursor(IDBKeyRange.lowerBound(opts.seq, true));
    ...
    var viewQuery = function(fun, idb, options) {
        ...
        function fetchMetadata(e) {
          ...
          var rev = winningRev(metadata);
          var dataReq = txn.objectStore(BY_SEQ_STORE).index('_id_rev').get(metadata.id + "::" + rev);
...
var webSqlPouch = function(opts, callback) {
  ...
  db.transaction(function (tx) {
    ...
    var seq = 'CREATE TABLE IF NOT EXISTS ' + BY_SEQ_STORE +
      ' (seq INTEGER NOT NULL PRIMARY KEY AUTOINCREMENT, id_rev UNIQUE, json)';
  ...
  api.bulkDocs = function idb_bulkDocs(req, opts, callback) {
    ...
    function writeDoc(docInfo, callback, isUpdate) {
      ...
      function finish() {
        ...
        var sql = 'INSERT INTO ' + BY_SEQ_STORE + ' (id_rev, json) VALUES (?, ?);';
        tx.executeSql(sql, [data._id + "::" + data._rev, JSON.stringify(data)], dataWritten);
      }
    }

    function updateDoc(oldDoc, docInfo) {
      var merged = Pouch.merge(oldDoc,
      ...
      if (docInfo.metadata.deleted) {
        if (docInfo && docInfo.metadata && !('deletions' in docInfo.metadata)) {
          docInfo.metadata.deletions = {};
        }
        docInfo.metadata.deletions[docInfo.metadata.rev.split('-')[1]] = true;
      }
  ...
  api.get = function(id, opts, callback) {
    ...
    db.transaction(function(tx) {
      ...
      tx.executeSql(sql, [id.docId], function(tx, results) {
        ...
        var rev = winningRev(metadata);
        var key = opts.rev ? opts.rev : rev;
        var sql = 'SELECT * FROM ' + BY_SEQ_STORE + ' WHERE id_rev=?';
        tx.executeSql(sql, [metadata.id + "::"+key], function(tx, results) {
          ...
          if (opts.revs_info) {
            ...
            while(node.pnode.parent){
              revisions2.push({rev:(node.treepos+node.pnode.depth-1)+"-" + node.pnode.node[0], status:"available"});
              node = {pnode:node.pnode.parent, treepos:node.treepos};
            }
            revisions2.push({rev:(node.treepos+node.pnode.depth-1)+"-" + node.pnode.node[0], status:"available"});
